...
def square_depth_image(depth_image):
    ...
    cropped = depth_image[:, 38:-38, :]  # Crop width: take center (300 → 200)
    ...
    return depth_resized.squeeze(0)#.permute(1,2,0)

# resizing the semantic map from 200x300 to 224x224
def square_semantic_class(semantic_class_img):
    # Input is (200,300,1)
    semantic_tensor = semantic_class_img[:, 38:-38,:]

    # Convert to torch tensor with shape (1, 1, H, W)
    semantic_tensor = torch.from_numpy(semantic_tensor).permute(2, 0, 1).unsqueeze(0).float()

    # Resize to (224, 224) using nearest neighbor interpolation
    semantic_tensor = F.interpolate(semantic_tensor, size=(224, 224), mode='nearest')

    # Return tensor of shape (224, 224)
    return semantic_tensor.squeeze(0)
...
def semantic_class_to_color(semantic_img):
    # Semantic labels
    semantic_colormap = {
        0: [0, 0, 0],         # UNLABELED
        1: [0, 0, 142],       # CAR
    ...
    for label, color in semantic_colormap.items():
        for i in range(3):
            rgb_semantic_img[i][semantic_img == label] = color[i] / 255.0
    return rgb_semantic_img
class DrivingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        semantic_map = data['semantic_label'] # values: 0-14
        # semantic_map = semantic_class_to_color(semantic_map)
        # semantic_map = square_semantic_map(semantic_map)
        semantic_map = square_semantic_class(semantic_map[..., None])
        semantic_map = semantic_map.to(torch.long)
